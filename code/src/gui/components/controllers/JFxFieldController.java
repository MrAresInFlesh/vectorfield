...
import gui.components.controllers.commandpanel.JFxFieldCommandPanel;
...
public class JFxFieldController extends VBox {

    /*------------------------------------------------------------------*|
	|*							Constructors							*|
	|*------------------------------------------------------------------*/

	public JFxFieldController () {
        super();
        isMovable = false;
        geometry();
        control();
	...
	public JFxFieldCommandPanel getPanel() {
		return panel;
	}
	...
	private void geometry() {
        ...
        paneBar = new JFxPaneBar(new Label("FIELD"), STG.BTN_SIZE20);
        paneBar.focusedProperty();

		panel = new JFxFieldCommandPanel();
		
		JFxVBox boxPaneBar = new JFxVBox(false);
		boxPaneBar.getChildren().addAll(paneBar);
		
		JFxVBox boxSettings = new JFxVBox(false);
		boxSettings.getChildren().addAll(panel);
		
		JFxVBox mainLayout = new JFxVBox(false);
	...
	// container
	private JFxFieldCommandPanel panel;
